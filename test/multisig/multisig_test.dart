...
void main() {
  group('Multisig scenarios', () {
    ...
    test(
      ...
      () {
        ...
        final isValidSig1 = k1.getPublicKey().verifyPersonalMessage(
          signData,
          sig1.signature,
        );
        final isValidSig2 = k2.getPublicKey().verifyPersonalMessage(
          signData,
          sig2.signature,
        );
        ...
        final bytes = SuiBcs.MultiSig.serialize({
          ...
          "bitmap": 5,
          "multisig_pk": SuiBcs.MultiSigPublicKey.parse(
    ...
    test('providing signatures with invalid order', () {
      ...
      expect(
        () => multiSigPublicKey.verifyPersonalMessage(signData, multisig),
        throwsException,
      );
    });
    ...
    test('providing invalid intent scope', () {
      ...
      // Invalid intentScope.
      expect(
        () => publicKey.verifyTransaction(signData, multisig),
        throwsException,
      );
      expect(
        () => multiSigPublicKey.verifyTransaction(signData, multisig),
        throwsException,
      );
    });
  ...
  group('Multisig address creation:', () {
    ...
    setUpAll(() {
      ...
      k6 = Ed25519Keypair.fromSecretKey(secretKeyEd25519);
      pk6 = k6.getPublicKey();
    });
    ...
    test('`toMultiSigAddress()` with zklogin identifiers', () {
      ...
      final multisigPublicKey = MultiSigPublicKey.fromPublicKeys(
        publicKeys: [PublicKeyWeight(1, pk4), PublicKeyWeight(1, pk5)],
        threshold: 1,
      );
    ...
    test('`combinePartialSigs()` with zklogin sigs', () {
      ...
      final publicKey = MultiSigPublicKey.fromPublicKeys(
        publicKeys: [PublicKeyWeight(1, pk6), PublicKeyWeight(1, pk4)],
        threshold: 1,
      );
      expect(
      ...
      final parsedZkloginSig = parseSerializedSignature(zkloginSig);
      final multisig = publicKey.combinePartialSignatures([
        sig1.signature,
        zkloginSig,
      ]);
      expect(
      ...
      final decoded = parsePartialSignatures(
        MultiSigStruct.fromJson(
      ...
      final origin1 = ParsedPartialMultiSigSignature(
        ...
        signature: parsedZkloginSig.signature,
        publicKey: pk4,
        weight: 1,
      );
    ...
    test('with empty values', () {
      expect(
        () => MultiSigPublicKey.fromPublicKeys(threshold: 2, publicKeys: []),
        throwsArgumentError,
      );
    });
